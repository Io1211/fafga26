...
import { bildHolen } from "./werkzeug.js";

export const MASSE = {
  story:  { w: 1080, h: 1920 },
  post:   { w: 1080, h: 1350 },
  square: { w: 1080, h: 1080 }   // wie backend/app/render.py
};
...
export async function zeichnePost(canvas, opts = {}) {
  ...
  if (im) {
    ...
    if (passung === "fuellend") {
      // Beschnitt am Fokuspunkt ausrichten, nicht stur mittig — sonst
      // schneidet ein hochkantes Motiv den Kopf ab. focal_x/y kommt vom
      // Backend mit, Vorgabe ist die Mitte.
      const cov = Math.max(W / im.width, H / im.height) * z;
      const w = im.width * cov, h = im.height * cov;
      const fx = (W - w) * (state.focalX ?? 0.5);
      const fy = (H - h) * (state.focalY ?? 0.5) - H * 0.02 * push;
